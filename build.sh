...
NODE_LIB_DIR='buildroot-config/board/rootfs_overlay/usr/local/lib/nodejs'
if [ ! -d $NODE_LIB_DIR ]
then
...
echo "./buildroot-config copying to ./buildroot-$BUILD_ROOT_RELEASE"

cp -fr ./buildroot-config ./buildroot-$BUILD_ROOT_RELEASE/
cd buildroot-$BUILD_ROOT_RELEASE

# Build our defconfig.
make BR2_EXTERNAL=buildroot-config defconfig && make
